<div>
                                                ...
                                                <div class="btn-toolbar justify-content-center" role="toolbar">
                                                    <div class="btn-group" role="group">
                                                        <button class="btn btn-sm btn-outline-secondary" type="button"
                                                            wire:click="showRoleById({{ $rol->id }})">
                                                        ...
                                                        @can('role-update')
                                                            <button type="button" class="btn btn-sm btn-outline-secondary"
                                                                wire:click="editRoleById({{ $rol->id }})">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                        ...
                                                        @can('role-destroy')
                                                            <button class="btn btn-sm btn-outline-secondary" type="button"
                                                                wire:click="deleteRoleById({{ $rol->id }})">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="16"
    ...
    <div wire:ignore.self class="modal fade" id="viewRole" tabindex="-1" aria-labelledby="ViewRole"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Ver Rol</h1>
                </div>
                <div class="modal-body">
                    <div class="row mb-3">
...
@push('scripts')
    <script>
        window.addEventListener('closeModal', event => {
            // document.getElementById('addNewRole').hide();
            // document.getElementById('editRole').hide();
            $('#viewRole').modal('hide');
            $('#addNewRole').modal('hide');
            $('#editRole').modal('hide');
            $('#destroyRole').modal('hide');
        });
        window.addEventListener('editRoleModal', event => {
            $("#editRole").modal('show');
        })
        window.addEventListener('DestroyRoleModal', event => {
            $("#destroyRole").modal('show');
        })
        window.addEventListener('showRoleModal', event => {
            $("#viewRole").modal('show');
        })
    </script>
@endpush
